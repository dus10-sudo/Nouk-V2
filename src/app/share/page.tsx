...
export default function SharePage() {
  ...
  useEffect(() => {
    async function loadRooms() {
      ...
      if (!roomSlug && typed.length > 0) {
        setRoomSlug(typed[0].slug);
      }
    }
  ...
  async function handleSubmit(e: React.FormEvent) {
    e.preventDefault();
    if (!roomSlug || (!title.trim() && !body.trim() && !link.trim())) return;
    ...
    try {
      ...
      // send them back to the selected category feed
    ...
    <main className="min-h-screen bg-[#f5eedf]">
      <div className="max-w-xl mx-auto px-4 pb-16 pt-8">
        <header className="mb-6 text-center">
          <h1 className="text-2xl font-semibold text-neutral-900">Start a Post</h1>
